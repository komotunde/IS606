...
```{r sample_distribution, eval=TRUE}
hist(samp, breaks = 20)
summary(samp)
```  
####*This distribution is multimodal, and not normal. It's kind of every where.*
...
####*The sample has to be random and independent. It must be a simple random sample with less than 10% of the population.*

## Confidence levels

4.  What does "95% confidence" mean? If you're not sure, see Section 4.2.2.  

####*A 95% confidence correlates with 2 standard errors. If our confidence interval spreads over those 2 standard errors from the point estimate, we are 95% sure that we captured the true parameter. From section 4.2.2, "if we took many samples and build confidence intervals from each, 95% of them would contain the actual mean."*  
...
####*I would expect about 95% of them to capturethe true population mean.*
...
upper_vector <- samp_mean + 2.5 * samp_sd/sqrt(n)
c(lower_vector[1], upper_vector[1])
plot_ci(lower_vector, upper_vector, mean(population))
```  
