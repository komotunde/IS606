...
-   What years are included in this data set? What are the dimensions of the 
    ...
    view(present)               #The years included are 1940-2002.
    dim(present)                #[1] 63  3    
...
-   How do these counts compare to Arbuthnot's? Are they on a similar scale?
    ...
    range(present$girls+present$boys)
    range(arbuthnot$girls+arbuthnot$boys)
    #The counts for present data set are significantlly higher.
...
-   Make a plot that displays the boy-to-girl ratio for every year in the data 
    ...
    #Boys are being born at a slightly higher rate than girls. 
...
-   In what year did we see the most total number of births in the U.S.? You can
    ...
    maxbirth <- df(present$year, present$boys + present$girls)
names(maxbirth) <- c("year", "totalBirths")
maxbirth <- maxbirth[order(maxbirth$totalBirths, decreasing = TRUE),]
head(maxbirth, n = 1)
#1961
