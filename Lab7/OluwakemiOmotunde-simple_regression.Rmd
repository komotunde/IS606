...
**It looks as though they have a positive, slightly strongth linear relationship.There also seems to be a few outliers.**
...
**It doesn't seem as though 'plot_ss' is interactive for me**
...
#regression line: y = 415.2389 + 1.8345 * homeruns
```
...
5.  If a team manager saw the least squares regression line and not the actual data, how many runs would he or she predict for a team with 5,578 at-bats? Is this an overestimate or an underestimate, and by how much? In other 
...
res. <- 713 - 727.6861
res.
#we observe a residual of -14.6861
```
...
**There is no obvious pattern in the residuals plot.**
...
**Yes, this condition seems to have been met.**

*Constant variability*:

8.  Based on the plot in (1), does the constant variability condition appear to  be met?

**Yes, the variablity condition seems to have been met as well**
...
```{r summaries}
summary(m1) #Multiple R-squared:  0.3729,	Adjusted R-squared:  0.3505
m2 <- lm(runs ~ hits, data = mlb11)
summary(m2) #Multiple R-squared:  0.6419,	Adjusted R-squared:  0.6292
...
**Based on the above, I would say that the hits are a better preditor since about 63% of the variability is said to be due to hits. This is only 35% for the at_bats variable.**
...
**Graphically, all except stolen bases seems to have good linearity. We will check and see which has the highest Rsquared value.**
...
summary(m5) #Multiple R-squared:  0.1694,	Adjusted R-squared:  0.1397
...
**Numerically, it seems that the best predictor is batting averagee. It has the highest R squared value.**

-   Now examine the three newer variables. These are the statistics used by the author of *Moneyball* to predict a teams success. In general, are they more  or less effective at predicting runs that the old variables? Explain using 
appropriate graphical and numerical evidence. Of all ten variables we've analyzed, which seems to be the best predictor of `runs`? Using the limited (or not so limited) information you know about these baseball statistics, does your result make sense?
...
**The all satisfy the linearity criteria. We will now check the R squared value for each to see how they fair against each other.**

```{r}
m8 <- lm(runs ~ new_onbase, data = mlb11)
summary(m8) #Multiple R-squared:  0.8491,	Adjusted R-squared:  0.8437 


m9 <- lm(runs ~ new_slug, data = mlb11)
summary(m9) #Multiple R-squared:  0.8969,	Adjusted R-squared:  0.8932 


m11 <- lm(runs ~ new_obs, data = mlb11)
summary(m11)#Multiple R-squared:  0.9349,	Adjusted R-squared:  0.9326 

```

-   Check the model diagnostics for the regression model with the variable you decided was the best predictor for runs.

**The best predictor seems to be the combination of on-base percentage and slugging percentages.**
