...
####*The distribution looks to be slightly skewed to the right, as well as bimodal. Because N is greater than 30, we can ignore the slight skew and say the distribution is normal.*  
...
```{r samp1-dist, eval=TRUE}
hist(samp1)
```
####*The for the sample is unimodal, and a right skew as well. The range of the sample distribution is less than that of the population. Each bin has a wider range in the population histogram (0-1000 vs. 500-1000).*
...
#1499.69
```
####*The mean of samp2 is less than that of samp1. If we took those two samples (100 and 1000), I would guess that sample size of 1000 would be more accuate.We can check this hypothesis below.*

```{r samp-mean, eval=TRUE}
...
######*There are 5000 elements in 'sample_means50'. The distribution is multimodal, centered at what looks to be about 1500. It looks to have a normal distribution. I would expect the distribution to become even more normally distributed and the center to be closer to the population mean.*
...
####*The center becomes closer to the population mean while the spread decreases, meaning it is not as spread out as the smaller sample size.*
* * *
## On your own
...
```{r samp_price, eval=FALSE}
samp50 <- sample(price, 50)
hist(samp50)
mean(samp50)
```
####*I would estimate that the point estimate of the population mean is 17,6567.1.*
...
for(i in 1:5000){
  ...
  mean(price)  
  #180796.1 is the population mean.
```
####*The distribution is multimodal, the spread is relatively close, centered around 180000. I would guess the mean to be around 180000.*
...
####*The shape of the distribution is normal. The distribution is multimodal centered about 180000. There are more entries making it more accuarate in guessing the population mean.*
...
####*The distribution from 3 contains a smaller spread to the distribution for 2,  because of this, we would prefer #3 to make estiamtes that are close to the tru value.*
