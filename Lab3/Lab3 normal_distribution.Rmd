...
```{r male1-female1, eval=TRUE}
hist(mdims$hgt, main = "Height of Men")
hist(fdims$hgt,  main = "Height of Women")
```
  #The histogram for men looks like a normal distribution; it is visually easy to see this. The one for women seems a little skew to the right. 
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
    distribution? 
    #Yes, it appears that it does follow a nearly normal distribution. 
...
deviation ofthe normal distribution from which the simulated sample will be 
...
3.  Make a normal probability plot of `sim_norm`.  Do all of the points fall on 
    ...
    data?
```{r sim-norm1, eval=TRUE}
qqnorm(sim_norm)
qqline(sim_norm)
```
#Not all points are on the line. About 10 of them aren't on the line. 
...
```{r qqnormsim1, eval=TRUE}
qqnormsim(fdims$wgt)
```
#Female weight does not appear to be a normal distribution.
...
#What is the probability on picking someone who weighs less than 67lbs?
```{r probability1, eval=TRUE }
fwgtmean <- mean(fdims$wgt)
fwgtsd <- sd(fdims$wgt)
pnorm(q = 67, mean = fwgtmean, sd = fwgtsd) 
sum(fdims$wgt < 67)/length(fdims$wgt)

#pnorm: 0.7471469, empirical: 0.7730769
```
#What is the probability of choosing someone with height greater than 169?
```{r probability2, eval=TRUE}
1-pnorm(q = 169, mean = fhgtmean, sd = fhgtsd)
sum(fdims$hgt > 169)/length(fdims$hgt)
#pnorm: 0.2641178, empirical: 0.2615385
#The heights have a closer agreement between pnorm and empirical
...
-   Note that normal probability plots C and D have a slight stepwise pattern.  
    ...
    #I would assume it is because the data jumps. 
