...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a streak of 1? What about a streak length of 0?
  #A streak of one indicates that there was one hit. The number of misses is also one. A streak length of 0 indicates that a shot was missed. 
...
#Typical length is 0, longest is 4.
...
4.  What change needs to be made to the `sample` function so that it reflects a shooting percentage of 45%? Make this adjustment, then run a simulation to 
...
-   Describe the distribution of streak lengths. What is the typical streak 
    length for this simulated independent shooter with a 45% shooting percentage? How long is the player's longest streak of baskets in 133 shots?

summary(sim_basket_streak)

#The distribution is skewed to the right. They typical streak lenth for this simulated independent shooter is 0. The longest streak is 5.
...
#I would expect the distribution to be similar to the first one. This is because we set the probability. They will not be exact but similar. 
...
summary(kobe_streak)
barplot(table(kobe_streak))
#They are similar in that they are both skewed right. The min, 1st quartile, mode, median, and 3rd quartile are the same (0,0,0,0,1). The max and mean are slightly different though. The max streak for the simulation is 5 while for Kobe, it is 4. The fact that the max streak for the simulation is higher, I would say that the hot hand model does not fit Kobe's shoot patterns.
<div id="license">
