...
-   Make a scatterplot of weight versus desired weight. Describe the 
    ...
    #These two variables have a positive linear relationship. 
...
-   What type of data is `wdiff`? If an observation `wdiff` is 0, what does
    ...
    #wdiff is an integer. 
    #When wdiff is 0, it means that the person's weight and desired weight are the same.
    #It wdiff is positve, the subject would like to increase their weight. If it is negative,
    #it means that they would like to lose weight.
...
-   Describe the distribution of `wdiff` in terms of its center, shape, and 
    ...
    hist(wdiff)
    #The distribution is slightly skewed to the left, unimodal, and centered around 0.
    #This tells us that they majority of peopel have no issues with their current weight.
...
-   Using numerical summaries and a side-by-side box plot, determine if men tend
    ...
    boxplot(wdiff ~ cdc$gender, outline = F,ylab = "wdiff"); 
    #We can determine that median weight difference for men is closer to 0. Therefore we can deduce
    #mean moreso that women, do not have issues with their weight.
...
-   Now it's time to get creative. Find the mean and standard deviation of 
    ...
    total <- sum(cdc$weight)
    patients <- 20000
    meanweight <- total/patients
    meanweight
    tail(cdc) 
    summary(cdc$weight) #to check and make sure above code was correct
    
    sd1 <- (cdc$weight - meanweight)^2
    meansd <- mean(sd1)
    sd <- sqrt(meansd)
    sd
    
    minus_sd <- cdc$weight - sd
    plus_sd <- cdc$weight + sd
    plot(plus_sd, minus_sd)
